  ...
  return (
    <div className="space-y-12 sm:space-y-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 max-w-7xl mx-auto">
        {features.map((feature, index) => {
          ...
          return (
            <div 
              key={index}
              className={`group relative ${feature.bgColor} backdrop-blur-sm border ${feature.borderColor} rounded-3xl p-8 sm:p-10 hover:scale-105 transition-all duration-300 hover:shadow-soft-lg animate-scale-in`}
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              {/* Icon */}
              <div className={`w-16 h-16 sm:w-18 sm:h-18 lg:w-20 lg:h-20 bg-gradient-to-r ${feature.gradient} rounded-3xl flex items-center justify-center mx-auto mb-6 sm:mb-8 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-soft-md`}>
              ...
              {/* Hover Effect */}
              <div className="absolute inset-0 bg-white/20 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 max-w-5xl mx-auto pt-8 sm:pt-12 animate-fade-in">
        <div className="text-center space-y-2 sm:space-y-3 p-6 bg-white/60 rounded-2xl border border-gray-100 shadow-soft">
